...
- **Calendar.** Only when `avoidCalendarConflicts`. Each busy block is widened by `bufferMinutes` on both sides; a buffer that just touches the slot doesn't clash. For a reschedule, the one busy block that exactly matches the existing appointment (same start, and the same end: its `endsAt`, else start plus the service's duration) is ignored; free/busy has no ids, so an exact match is the best signal. Order doesn't matter: a clash with any block counts. A busy block that can't be read means the slot can't be accepted (`calendar_unreadable`): the rule can't be checked, so code doesn't guess.

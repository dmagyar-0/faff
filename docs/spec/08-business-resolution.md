...
2. The agent calls `confirm_business_identity(heard_name, heard_location)`, which runs `matchBusinessIdentity` in `core` (G9). It compares words, after case-folding and removing generic ones ("the", "practice", "surgery", "ltd"…), with the display name, the part before its comma, the name without its location, and each alias. Every word of the name must be heard: exactly, with one slip for longer words that start with the same letter, or as heard words run together ("smile dent all" is Smile Dental). All heard and nothing else distinctive is a match; an extra distinctive word ("Smile Dental Balham") or only some of the words is unclear; none is a mismatch. A heard location must match too. It errs towards unclear, which costs one question, because a match lets `reveal_profile_field` release the user's details. The tolerance is provisional until the M6 noise scenarios tune it.
